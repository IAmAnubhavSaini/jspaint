/*! jspaint 2015-10-06 Anubhav Saini (c) 2015 */
...
},stop:function(a){$("#"+a.canvasId).off(a.event)},ContextMenu:{activate:function(b){function c(b,c,d,e){return a.generateSlider({id:b,min:c,max:d,step:1,title:e})}function d(b){function d(a){var b=c("organismPointWalkerSptes",m.VARIABLES.steps,a.maxStepsAllowed,"Steps for organism random point walk generation.").attr("value",m.VARIABLES.steps).on("mouseover",function(){$(this).attr("title",$(this).val())}).on("change",function(){m.VARIABLES.steps=$(this).val()});return b}return a.genericLabel().append(b.stepLabel).append(d(b))}var e=$("<div></div>").attr("id",b.id).addClass("menu-item");e.append(d(b)),e.appendTo($(b.containerSelectionCriterion))},deactivate:function(a){$("#"+a.id).remove()},getOptions:function(){return{tool:this,id:"OrganismPointWalkerContextMenu",containerSelectionCriterion:".contextual-tool-bar",maxStepsAllowed:1e4,stepLabel:"Steps: "}}}};a.registerEventForTool({toolId:m.CONSTANTS.selectionId,event:CONSTANTS.Events.mouseclick,canvasId:CONSTANTS.canvasId,start:m.start,stop:m.stop,toolName:"Organism Point Walker",contextMenu:m.ContextMenu,constantTitle:m.CONSTANTS.title}),$("#SaturateRedColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]=255;context.putImageData(d,0,0)}),$("#SaturateGreenColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=1;e<d.data.length;e+=4)d.data[e]=255;context.putImageData(d,0,0)}),$("#SaturateBlueColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=2;e<d.data.length;e+=4)d.data[e]=255;context.putImageData(d,0,0)}),$("#InvertColorsTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]=255-d.data[e],d.data[e+1]=255-d.data[e+1],d.data[e+2]=255-d.data[e+2];context.putImageData(d,0,0)}),$("#DesaturateRedColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]=0;context.putImageData(d,0,0)}),$("#DesaturateGreenColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=1;e<d.data.length;e+=4)d.data[e]=0;context.putImageData(d,0,0)}),$("#DesaturateBlueColorTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=2;e<d.data.length;e+=4)d.data[e]=0;context.putImageData(d,0,0)}),$("#AddGrayTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]+=112,d.data[e+1]+=112,d.data[e+2]+=112;context.putImageData(d,0,0)}),$("#RemoveGrayTool").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]-=112,d.data[e+1]-=112,d.data[e+2]-=112;context.putImageData(d,0,0)}),$("#AddNoise").on("click",function(){for(var a="#"+CONSTANTS.canvasId,b=$(a).height(),c=$(a).width(),d=context.getImageData(0,0,c,b),e=0;e<d.data.length;e+=4)d.data[e]+=Math.random()<.5?255*Math.random()*-1:255*Math.random(),d.data[e+1]+=Math.random()<.5?255*Math.random()*-1:255*Math.random(),d.data[e+2]+=Math.random()<.5?255*Math.random()*-1:255*Math.random();context.putImageData(d,0,0)})});