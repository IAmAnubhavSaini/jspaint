...
function generateLabelString(options) {
...
$(function () {
    ...
    var COMMON = {
        ...
        registerEventForTool: function (options) {
            var toolId = options.toolId, tool = $(toolId), contextMenu = options.contextMenu, title = options.constantTitle;
    ...
    var MandelbrotFractalFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = options.canvasId;
            function drawMandelbrotFractal(options) {
                function mandelIter(cx, cy, maxIter) {
                    var x = 0.0, y = 0.0, xx = 0, yy = 0, xy = 0, i = maxIter;
                    while (i-- && xx + yy <= 4) {
                        xy = x * y;
                        xx = x * x;
                        yy = y * y;
                ...
                function mandelbrot(options) {
                    var ctx = options.context, xmin = options.XMin, ymin = options.YMin, xmax = options.XMax, ymax = options.YMax, iterations = options.iterations, width = MandelbrotFractal.VARIABLES.width, height = MandelbrotFractal.VARIABLES.height, img = ctx.getImageData(0, 0, width, height), pix = img.data, innerColor = Color.hexToRgb(selectedPrimaryColor), ix, iy, x, y, i, c, ppos;
            ...
            var getStartingXCoordinate = function (mouseOptions) {
                var X = Actions.Mouse.getX(mouseOptions), startX = Math.max(X - Math.floor(MandelbrotFractal.VARIABLES.width / 2), 0), overflowX = getOverflowInXAxis(startX);
                if (overflowX > 0) {
                    startX -= overflowX;
                }
                return startX;
            ...
            var getStartingYCoordinate = function (mouseOptions) {
                var Y = Actions.Mouse.getY(mouseOptions), startY = Math.max(Y - Math.floor(MandelbrotFractal.VARIABLES.height / 2), 0), overflowY = getOverflowInYAxis(startY);
                if (overflowY > 0) {
                    startY -= overflowY;
                }
                return startY;
            ...
            $(canvasId).on(event, function (e) {
                var mouseOptions = {
                    event: e,
                    relativeTo: $(canvasId)
                }, startX = getStartingXCoordinate(mouseOptions), startY = getStartingYCoordinate(mouseOptions);
        ...
        Events: {
            register: function (options) {
                var toolId = options.toolId, tool = $(toolId), contextMenu = MandelbrotFractal.ContextMenu;
    ...
    var PencilFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = '#' + options.canvasId, mouseOptions = null, X = null, Y = null, width = null, last = null, LastPoint = {
                get: function () {
                    return {
    ...
    var PivotedLinePatternFunctionality = {
        start: function (options) {
            var event = options.event || CONSTANTS.Events.mousemove, canvasId = '#' + (options.canvasId || CONSTANTS.canvasId), mouseOptions = null, X = null, Y = null, width = null, last = null, LastPoint = {
                get: function () {
                    return {
    ...
    var RectangleFunctionality = {
            ...
            function generatePreview(options) {
                var div = $('<div></div>')
                    ...
                    var mouseOptions = {
                        event: eClick,
                        relativeTo: $(canvasId)
                    }, X = Actions.Mouse.getX(mouseOptions), Y = Actions.Mouse.getY(mouseOptions), length = Rectangle.VARIABLES.length, breadth = Rectangle.VARIABLES.breadth;
        ...
        stop: function (options) {
            var event = options.event || CONSTANTS.Events.mouseclick, canvasId = '#' + (options.canvasId || CONSTANTS.canvasId);
            $(canvasId).off(event);
        ...
        ContextMenu: {
            activate: function (options) {
                ...
                function addSliderForRadius(options) {
                    var div = $('<div></div>').attr('id', options.id).addClass('menu-item'), innerSlider = initialSlider('innerRadiusRing', 'inner radius for ring tool.')
    ...
    var DiscFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = '#' + options.canvasId, mouseOptions = null, X = null, Y = null, radius = null, previewer = null, canvasOffsetLeft = $(canvasId).offset().left, canvasOffsetTop = $(canvasId).offset().top, canvasHeight = $(canvasId).height(), canvasWidth = $(canvasId).width(), previewOffsetLeft = null, previewOffsetTop = null;
            function generatePreview(options) {
                var div = $('<div></div>')
            ...
            function generatePreview(options) {
                var div = $('<div></div>')
                    ...
                    var mouseOptions = {
                        event: eClick,
                        relativeTo: $(canvasId)
                    }, X = Actions.Mouse.getX(mouseOptions), Y = Actions.Mouse.getY(mouseOptions), side = Square.VARIABLES.side, xyPlaneRotationAngle = (Square.VARIABLES.xyPlaneRotationAngle * Math.PI) / 180;
    ...
    var CircleFunctionality = {
        ...
        ContextMenu: {
            activate: function (options) {
                ...
                function addSliderForRadius(options) {
                    var div = $('<div></div>').attr('id', options.id).addClass('menu-item'), radiusSlider = initialSlider('radiusCircle', 'innerRadius for circle tool.')
    ...
    var PointWalkerFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = '#' + options.canvasId, mouseOptions = null, X = null, Y = null, i = 0;
            $(canvasId).on(event, function (e) {
                mouseOptions = {
                    event: e,
                    relativeTo: $(canvasId)
    ...
    var FamilyPointWalkerFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = '#' + options.canvasId, mouseOptions = null, X = null, Y = null, i = 0, oldXY = [], origin = {}, steps, fillColor;
            $(canvasId).on(event, function (e) {
                mouseOptions = {
                    event: e,
                    relativeTo: $(canvasId)
        ...
        ContextMenu: {
            activate: function (options) {
                ...
                function addStepController(options) {
                    function createStepSlider(options) {
                        var slider = getInputElement('familyPointWalkerSptes', FamilyPointWalker.VARIABLES.steps, options.maxStepsAllowed, 'Steps for family random point walk generation.')
    ...
    var UniCellularParasiteToolFunctionality = {
        start: function (options) {
            var event = options.event, canvasId = '#' + options.canvasId, mouseOptions = null, X = null, Y = null, i = 0, origin = {}, steps, fillColor;
            $(canvasId).on(event, function (e) {
                mouseOptions = {
                    event: e,
                    relativeTo: $(canvasId)
    ...
    function onSaturateRedColorToolClick() {
        var canvasId = '#' + CONSTANTS.canvasId, height = $(canvasId).height(), width = $(canvasId).width(), image = context.getImageData(0, 0, width, height);
        saveCanvasState({
            startX: 0,
            startY: 0,
            width: width,
    ...
    function onRandomColorToolClick() {
        var canvas = getCanvasDetails(), image = canvas.image, sampleX = Math.floor(Math.random() * canvas.width), sampleY = Math.floor(Math.random() * canvas.height), sampleRed = image.data[sampleX * canvas.width + sampleY], sampleGreen = image.data[sampleX * canvas.width + sampleY + 1], sampleBlue = image.data[sampleX * canvas.width + sampleY + 2], red = Math.random() < 0.5 ? Math.random() * 255 * -1 : Math.random() * 255, green = Math.random() < 0.5 ? Math.random() * 255 * -1 : Math.random() * 255, blue = Math.random() < 0.5 ? Math.random() * 255 * -1 : Math.random() * 255;
    ...
    function onBlackAndWhiteColorToolClick() {
        var canvas = getCanvasDetails(), image = canvas.image, average = 0, newValue = 0;
        saveCanvasState(canvas);
        for (var i = 0; i < image.data.length; i += 4) {
    ...
    function onRandomDisksColorToolClick() {
        var canvas = getCanvasDetails(), savedStrokeStyle = canvas.strokeStyle;
        saveCanvasState(canvas);
    ...
    function onRandomCirclesColorToolClick() {
        var canvas = getCanvasDetails(), savedStrokeStyle = canvas.strokeStyle;
        saveCanvasState(canvas);
        function circleDrawOperation(x, y) {
            var innerRadius = Math.floor(Math.random() * 10), i = Math.floor(Math.random() * 16) || 0, color = CONSTANTS.basicColors[i] || CONSTANTS.defaultColor, strokeStyle = '#' + color.hex;
            CANVASAPI.drawCircle({
                X: x,
                Y: y,
                innerRadius: innerRadius,
