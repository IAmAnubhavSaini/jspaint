$(function() {
    ...
    $('#NerdStatTool').on('click', function() {
        $('#stats4nerds #stats').html();
        var canvas = $('#' + CONSTANTS.canvasId);
        var image = context.getImageData(0, 0, canvas.width(), canvas.height());
        var equalRedPixels = Stats4Nerds.onlyRedPixels(image.data);
        var equalGreenPixels = Stats4Nerds.onlyGreenPixels(image.data);
        var equalBluePixels = Stats4Nerds.onlyBluePixels(image.data);
        localStorage.setItem('RED', equalRedPixels);
        localStorage.setItem('GREEN', equalGreenPixels);
        localStorage.setItem('BLUE', equalBluePixels);
        $('#stats4nerds').show();

        var n = 3, // number of layers
            m = 256, // number of samples per layer
        ...
        var margin = {
            ...
            width = $('#stats4nerds').width() - margin.left - margin.right,
