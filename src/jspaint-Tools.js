$(function() {
    ...
    CONSTANTS: {
      ...
      function drawMandelbrotFractal(options) {
        ...
        function mandelbrot(options) {
          let ctx = options.context, xmin = options.XMin, ymin = options.YMin, xmax = options.XMax, ymax = options.YMax,
            iterations = options.iterations, width = MandelbrotFractal.VARIABLES.width,
            height = MandelbrotFractal.VARIABLES.height, innerColor = hex2RGB(window.JSPAINT.selectedPrimaryColor),
            altColor = hex2RGB(window.JSPAINT.selectedAlternativeColor), ix, iy, x, y, i, c, ppos;
      ...
      activate: function(options) {
        function getInputElement(id, min, max, title) {
          return COMMON.generateSlider({
            id: id, min: min, max: max, step: 1, title: title
          });
        }
  ...
  const Pencil = {
    CONSTANTS: {
      ...
      var event = options.event, canvasId = "#" + options.canvasId, mouseOptions = null, X = null, Y = null,
        width = null, last = null, LastPoint = {
          get: function() {
            return {
      ...
      activate: function(options) {
        ...
        function addSliderForLineWidth(options) {
          var div = $("<div></div>")
            .attr("id", options.id)
            .addClass("menu-item"), slider = initialSlider()
  ...
  const PivotedLinePattern = {
    CONSTANTS: {
      ...
      var event = options.event || CONSTANTS.Events.mousemove,
        canvasId = "#" + (options.canvasId || CONSTANTS.canvasId), mouseOptions = null, X = null, Y = null,
        width = null, last = null, LastPoint = {
          get: function() {
            return {
      ...
      activate: function(options) {
        ...
        function initialSlider() {
          return COMMON.generateSlider({
            id: "widthPivotedLinePattern", min: 1, max: 200, step: 1, title: "width for pivoted line pattern tool."
          });
        }
      ...
      deactivate: function(options) {
        ...
        return {
          tool: this, id: "PivotedLinePatternContextMenu", containerSelectionCriterion: ".contextual-tool-bar"
        };
      }
    }
  };
  ...
  var Rectangle = {
    CONSTANTS: {
      ...
      var event = options.event, canvasId = "#" + options.canvasId, mouseOptions = null, X = null, Y = null,
        length = null, breadth = null, previewer = null, canvasOffsetLeft = $(canvasId).offset().left,
        canvasOffsetTop = $(canvasId).offset().top, canvasHeight = $(canvasId).height(),
        canvasWidth = $(canvasId).width(), previewOffsetLeft = null, previewOffsetTop = null,
        ...
        var div = $("<div></div>")
          ...
          .on("click", function(eClick) {
            var mouseOptions = {
              ...
              }, X = Actions.Mouse.getX(mouseOptions), Y = Actions.Mouse.getY(mouseOptions),
              length = Rectangle.VARIABLES.length, breadth = Rectangle.VARIABLES.breadth;
  ...
  var Ring = {
    CONSTANTS: {
      ...
      var event = options.event || CONSTANTS.Events.mouseclick,
        canvasId = "#" + (options.canvasId || CONSTANTS.canvasId), mouseOptions = null, X = null, Y = null,
        radius = null, innerRadius = null, outerRadius = null, previewer = null, canvasOffsetLeft = null,
        canvasOffsetTop = null, canvasHeight = $(canvasId).height(), canvasWidth = $(canvasId).width(),
        previewOffsetLeft = null, previewOffsetTop = null, outer = null;
      ...
      activate: function(options) {
        function initialSlider(id, title) {
          return COMMON.generateSlider({
            id: id, min: 1, max: 200, step: 1, title: title
          });
        }
        ...
        function addSliderForRadius(options) {
          var div = $("<div></div>")
            .attr("id", options.id)
            .addClass("menu-item"), innerSlider = initialSlider("innerRadiusRing", "inner radius for ring tool.")
  ...
  var Disc = {
    CONSTANTS: {
      ...
      var event = options.event, canvasId = "#" + options.canvasId, mouseOptions = null, X = null, Y = null,
        radius = null, previewer = null, canvasOffsetLeft = $(canvasId).offset().left,
        canvasOffsetTop = $(canvasId).offset().top, canvasHeight = $(canvasId).height(),
        canvasWidth = $(canvasId).width(), previewOffsetLeft = null, previewOffsetTop = null;
  ...
  var Square = {
    CONSTANTS: {
      ...
      var event = options.event || CONSTANTS.Events.mouseclick,
        canvasId = "#" + (options.canvasId || CONSTANTS.canvasId), mouseOptions = null, X = null, Y = null, side = null,
        xyPlaneRotationAngle = null, previewer = null, canvasOffsetLeft = $(canvasId).offset().left,
        canvasOffsetTop = $(canvasId).offset().top, canvasHeight = $(canvasId).height(),
        canvasWidth = $(canvasId).width(), previewOffsetLeft = null, previewOffsetTop = null;
      ...
      function generatePreview(options) {
        var div = $("<div></div>")
          ...
          .on("click", function(eClick) {
            var mouseOptions = {
                event: eClick, relativeTo: $(canvasId)
              }, X = Actions.Mouse.getX(mouseOptions), Y = Actions.Mouse.getY(mouseOptions), side = Square.VARIABLES.side,
      ...
      activate: function(options) {
        ...
        function getContextMenuContainer(options) {
          ...
          if (container.length === 0) return $("<div></div>")
            .attr("id", options.id)
            .addClass("menu-item"); else return container;
        }
  ...
  var Circle = {
    CONSTANTS: {
      ...
      var event = options.event || CONSTANTS.Events.mouseclick,
        canvasId = "#" + (options.canvasId || CONSTANTS.canvasId), mouseOptions = null, X = null, Y = null,
        innerRadius = null, outerRadius = null, previewer = null, canvasOffsetLeft = null, canvasOffsetTop = null,
        canvasHeight = $(canvasId).height(), canvasWidth = $(canvasId).width(), previewOffsetLeft = null,
    ...
    CONSTANTS: {
      ...
      $(canvasId).on(event, function(e) {
        ...
        function addCircle(X, Y, api, i, max, color) {
          ...
          if (i < max) {
            setTimeout(() => addCircle(X, Y, CANVASAPI, i + 1, max, color), 1);
          }
        }

        addCircle(X, Y, CANVASAPI, 0, PointWalker.VARIABLES.steps, color);
      });
    }, stop: function(options) {
      $("#" + options.canvasId).off(options.event);
    }, ContextMenu: {
  ...
  var FamilyPointWalker = {
    CONSTANTS: {
      ...
      $(canvasId).on(event, function(e) {
        ...
        Y = Actions.Mouse.getY(mouseOptions);
        origin = {
          X: X, Y: Y, steps: FamilyPointWalker.VARIABLES.steps, fillColor: window.JSPAINT.selectedPrimaryColor
        };
  ...
  var OrganismPointWalker = {
    CONSTANTS: {
      ...
      $(canvasId).on(event, function(e) {
        ...
        Y = Actions.Mouse.getY(mouseOptions);
        origin = {
          X: X, Y: Y, steps: OrganismPointWalker.VARIABLES.steps, fillColor: window.JSPAINT.selectedPrimaryColor
        };
  ...
  $("#RandomColorTool").on("click", function() {
    var canvasId = "#" + CONSTANTS.canvasId, height = $(canvasId).height(), width = $(canvasId).width(),
      ...
      sampleGreen = image.data[sampleX * width + sampleY + 1], sampleBlue = image.data[sampleX * width + sampleY + 2],
